...
def get_connection_manager(request: Request) -> "ConnectionManager":
    ...
    if not hasattr(request.app.state, 'camera_connection_manager'):
        request.app.state.camera_connection_manager = ConnectionManager(request.app.state.detection_service)
    return request.app.state.camera_connection_manager
...
async def camera_stream(
    ...
    try:
        ...
        while True:
            ...
            # 执行检测
            try:
                detection_result = await detection_service.process_camera_frame(frame)
            except Exception as e:
            ...
            # 控制帧率
            await asyncio.sleep(0.05)  # 约20 FPS
